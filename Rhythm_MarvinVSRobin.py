...
(x, sr) = librosa.load('Got To Give lt Up - Marvin Gaye2.wav')  # x = a matriz de áudio e sr=taxa de amostragem
ipd.Audio(x, rate=sr)

# Extrair componente de percussão

x_percussive = librosa.effects.percussive(x, margin=20.0)
...
for it in range(1, numIter):
    ...
    plt.savefig('ultimaIterOriginalMarvin.png')
    plt.show()
...
(a, sr) = librosa.load('Robin Thicke - Blurred Lines ft. Pharrell.wav')  # x = a matriz de áudio e sr=taxa de amostragem
ipd.Audio(a, rate=sr)

# Extrair componente de percussão

a_percussive = librosa.effects.percussive(a, margin=20.0)
...
for it in range(1, numIter):
    ...
    plt.gca().set_color_cycle(['red', 'blue', 'green'])
    ax0.plot(t, amplitude_envelope)
    plt.savefig('ultimaIterPlagioRobin.png')
    plt.show()
...
for n in range(numComp):
    ...
    plt.plot(G1[n] * 8, label='Amostra original')
    plt.plot(G[n], label='Amostra suspeita ')
    plt.ylim(0, max((G1[n] * 8).max(), G[n].max()))
    plt.xlim(0, G.shape[1])
    plt.legend(bbox_to_anchor=(1.0, 1), loc=1, borderaxespad=0.)
    plt.ylabel('Componente %d' % n)
    plt.savefig('marvinerobincomp.png')

# Autocorrelação amostra original

(fig, axes) = plt.subplots(nrows=1)
acorr_results = axes.acorr(x_percussive)
r_source = acorr_results[1]
plt.savefig('correlacao_originalmarvin.png')

# Autocorrelação amostra suspeita

(fig, axes) = plt.subplots(nrows=1)
acorr_results = axes.acorr(a_percussive)
r_plag = acorr_results[1]
plt.savefig('correlacao_suspeitarobin.png')
